...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		$lazyload_choices = [
			'lazyload'    => __( 'Lazy-load on (default)', 'ucf-bands' ),
			'no-lazyload' => __( 'Lazy-load off', 'ucf-bands' ),
		];
		...
		$wp_customize->add_control(
			'lazy_load_media',
			[
				'label'           => __( 'Lazy-load images', 'ucf-bands' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'ucf-bands' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'ucf-bands-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			[],
			ucf_bands()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'ucf-bands-lazy-load-images', 'defer', true );
		wp_script_add_data( 'ucf-bands-lazy-load-images', 'precache', true );
	}
