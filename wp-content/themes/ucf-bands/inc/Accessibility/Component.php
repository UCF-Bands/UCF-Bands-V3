...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'ucf-bands-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			ucf_bands()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'ucf-bands-navigation', 'async', true );
		wp_script_add_data( 'ucf-bands-navigation', 'precache', true );
		wp_localize_script(
			'ucf-bands-navigation',
			'ucfBandsScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'ucf-bands' ),
				'collapse' => __( 'Collapse child menu', 'ucf-bands' ),
			]
		);
	}
