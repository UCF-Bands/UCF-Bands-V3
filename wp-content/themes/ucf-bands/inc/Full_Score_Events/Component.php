...
class Component implements Component_Interface {
	...
	public function initialize() {
		...
		add_filter( 'ucf_bands_css_files', [ $this, 'add_css_files' ] );
	...
	public function add_css_files( $css_files ) {
		...
		$css_files['ucf-bands-fse'] = [
			'file' => 'full-score-events/global.min.css',
		];

		$css_files['ucf-bands-fse-events'] = [
			'file' => 'full-score-events/events.min.css',
		];

		$css_files['ucf-bands-fse-event'] = [
	...
	public function do_template_styles() {
		...
		if ( is_event_archive() ) {
			ucf_bands()->print_styles( 'ucf-bands-fse-events' );
		} elseif ( is_event() ) {
			ucf_bands()->print_styles( 'ucf-bands-fse-event' );
		}
	}
	...
	public function action_enqueue_scripts() {
		...
		wp_enqueue_script(
			...
			[],
			ucf_bands()->get_asset_version( get_theme_file_path( '/assets/js/full-score-events.min.js' ) ),
			false
		);
		wp_script_add_data( 'ucf-bands-fse', 'async', true );
		wp_script_add_data( 'ucf-bands-fse', 'precache', true );
	}
