...
if ( 'post' === $this_post_type || get_post_type_object( $this_post_type )->has_archive ) {
	the_post_navigation(
		[
			'prev_text' => '<div class="post-navigation-sub"><span class="icon-link">' . esc_html__( 'Previous:', 'ucf-bands' ) . '</span></div>%title',
			'next_text' => '<div class="post-navigation-sub"><span class="icon-link">' . esc_html__( 'Next:', 'ucf-bands' ) . '</span></div>%title',
		]
	);
}
